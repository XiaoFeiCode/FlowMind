...
目录结构：
    ...
    ├── requirements-flowmind.txt     <-- 依赖列表也放在这里
    ├── flowmind/                  <-- 要安装的 Python 包
    │   ├── __init__.py
    │   ├── agent/
    │   ├── core/
    │   └── ...
...
- flowmind/ 是我们要安装的包（里面有 __init__.py）
- 所以 setup.py 放在 flowmind/ 的父目录，即项目根目录
...
2. flowmind 包会被注册到 Python 环境，可以在任何地方 import
3. flowmind 命令会被注册到系统，可以在命令行直接使用：
   - flowmind init    # 初始化项目
   - flowmind train   # 训练模型
   - flowmind run     # 启动服务
   - flowmind inspect # 交互式测试
...
Q: 安装后怎么卸载？
A: pip uninstall flowmind

Q: 怎么查看是否安装成功？
A: pip show flowmind  或者  python -c "import flowmind; print(flowmind.__version__)"
"""
...
def read_version() -> str:
    """从 flowmind/__init__.py 读取版本号。
    
    为什么版本号定义在 __init__.py 而不是这里？
    - 这样代码里可以通过 flowmind.__version__ 获取版本
    - 版本号只需要维护一处，不会出现不一致
    """
    init_path = Path(__file__).parent / "flowmind" / "__init__.py"
...
# 核心依赖：运行 flowmind 必须的包
# 从 requirements-flowmind.txt 文件读取
install_requires = read_requirements("requirements-flowmind.txt")
...
setup(
    ...
    # 
    # include: 只包含这些包（flowmind 和它的所有子包）
    ...
    # 命令行入口点（CLI）
    # ------------------------------------------------------------------------
    # 安装后会创建 flowmind 命令，执行时调用 flowmind.cli 模块的 main 函数
    # 
    # 格式："命令名=模块路径:函数名"
    # 安装后可以直接在终端运行：flowmind init / flowmind train / flowmind run
    entry_points={
        "console_scripts": [
            "flowmind=flowmind.cli:main",
        ],
    },
