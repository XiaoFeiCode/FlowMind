...
def train_command(
    ...
    示例:
        flowmind train
        flowmind train --data ./my_data --output ./my_models
        flowmind train --dry-run  # 仅验证
    """
    verbose = ctx.obj.get("verbose", False)
    debug = ctx.obj.get("debug", False)
    
    click.echo("=" * 50)
    click.echo("FlowMind AI - 模型训练")
    click.echo("=" * 50)
    ...
    try:
        ...
        from flowmind.shared.config import FlowMindConfig
        
        # 加载配置（用于验证）
        click.echo("加载配置...")
        flowmind_config = FlowMindConfig.load(str(config_path))
        click.echo("  配置加载成功")
        ...
        if dry_run:
            ...
            from flowmind.dialogue_understanding.flow import FlowLoader
